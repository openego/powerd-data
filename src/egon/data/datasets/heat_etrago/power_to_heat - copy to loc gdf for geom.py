...
def insert_power_to_heat_per_level(
    ...
    print("q_geom_buses", q)
    print("geom_buses head", geom_buses.head(10))

    # Create topology of heat pumps
    print("len(geom_buses)", len(geom_buses))
    print("len gdf", len(gdf))

    print("gdf_index", gdf.index)
    print("gdf_power_bus", gdf.power_bus)
    print("geom_buses_index", geom_buses.index)

    geom_buses_gdf = geom_buses.copy()
    try:
        geom_buses_gdf = geom_buses_gdf.loc[gdf.power_bus]
        print("got via geom_buses_gdf.loc[gdf.power_bus]")
    except Exception as E:
        geom_buses_gdf = geom_buses_gdf.loc[gdf.power_bus.to_list()]
        print("got via geom_buses_gdf.loc[gdf.power_bus.to_list()]")

    gdf["geom_power"] = geom_buses_gdf.geom[gdf.power_bus].values
    gdf["geom_heat"] = geom_buses_gdf.loc[gdf.heat_bus, "geom"].reset_index().geom
